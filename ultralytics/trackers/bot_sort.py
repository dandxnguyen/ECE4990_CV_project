...
class BOTSORT(BYTETracker):
    ...
    def _ocm_cost_matrix(self, tracks, detections, eps: float = 1e-6) -> np.ndarray:
        """
        Simple OCM-style motion cost.

        For each track, estimate motion direction from its last two centers.
        For each detection, compute direction from the last track center to the detection center.
        Cost is normalized angle difference in [0, 1].
        """
        if len(tracks) == 0 or len(detections) == 0:
            return np.zeros((len(tracks), len(detections)), dtype=np.float32)

        # 1) Direction of track motion from center_history
        track_dirs = []
        last_centers = []
        for trk in tracks:
            hist = getattr(trk, "center_history", None)
            if hist is None or len(hist) < 2:
                track_dirs.append(None)
                last_centers.append((0.0, 0.0))
                continue
            ...
            if abs(dx) < eps and abs(dy) < eps:
                ...
                track_dirs.append(math.atan2(dy, dx))
            last_centers.append((x2, y2))
        ...
        cost = np.zeros((num_t, num_d), dtype=np.float32)

        for i in range(num_t):
            theta_t = track_dirs[i]
            if theta_t is None:
                # no motion info -> no penalty
                continue

            cx_last, cy_last = last_centers[i]
            for j in range(num_d):
                cx_det, cy_det = det_centers[j]
                dx = cx_det - cx_last
                dy = cy_det - cy_last
                if abs(dx) < eps and abs(dy) < eps:
                    angle_diff = 0.0
                    ...
                    angle_diff = abs(theta_t - theta_d)
                    # wrap to [0, pi]
                    while angle_diff > math.pi:
                        angle_diff -= 2.0 * math.pi
                    angle_diff = abs(angle_diff)
                cost[i, j] = angle_diff / math.pi  # normalize [0,1]

        return cost

    
    def get_dists(self, tracks, detections):
        # 1) IoU / HMIoU distance (you already changed matching.iou_distance)
        dists = matching.iou_distance(tracks, detections)

        # 2) Add OCM motion-direction cost
        ocm_cost = self._ocm_cost_matrix(tracks, detections)
        print("OCM cost shape:", ocm_cost.shape)

        # You can put this in botsort.yaml as ocm_weight: 0.3
        w_ocm = getattr(self.args, "ocm_weight", 0.3)
        dists = dists + w_ocm * ocm_cost

        # 3) Original BoT-SORT logic
        dists_mask = dists > (1 - self.proximity_thresh)

        if self.args.fuse_score:
            dists = matching.fuse_score(dists, detections)

        if self.args.with_reid and self.encoder is not None and len(detections) and len(tracks):
